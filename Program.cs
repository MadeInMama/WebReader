...
builder.Services.AddRateLimiter(options =>
{
    ...
    builder.WebHost.ConfigureKestrel(serverOptions =>
    {
        ...
        {
            // Отключаем устаревшие протоколы TLS 1.0 и 1.1
            listenOptions.SslProtocols = SslProtocols.Tls12 | SslProtocols.Tls13;

            listenOptions.OnAuthenticate = (_, sslOptions) =>
            {
                // Задаем строгую политику шифров без CBC для Linux OpenSSL
                sslOptions.CipherSuitesPolicy = new TlsCipherSuitesPolicy(new[]
                {
                    // TLS 1.3
                    TlsCipherSuite.TLS_AES_256_GCM_SHA384,
                    TlsCipherSuite.TLS_CHACHA20_POLY1305_SHA256,
                    TlsCipherSuite.TLS_AES_128_GCM_SHA256,
                    // TLS 1.2 (Только безопасные GCM и CHACHA20)
