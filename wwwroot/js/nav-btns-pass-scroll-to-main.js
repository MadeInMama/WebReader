...
function passScrollToMain(el) {
    ...
    // ИЗМЕНЕНИЕ: Убрали passive: true, чтобы иметь возможность сделать preventDefault при необходимости
    ...
    // ИЗМЕНЕНИЕ: Добавлен event.preventDefault() для блокировки нативного двойного скролла
    el.addEventListener('touchmove', (event) => {
        const touch = Array.from(event.touches).find(t => t.identifier === primaryTouchId);
        if (!touch) return;

        // Блокируем нативное движение экрана, передавая управление вашему скрипту 1:1
        if (event.cancelable) {
